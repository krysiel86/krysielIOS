...
- (instancetype)initWithConnection:(id<PrebidServerConnectionProtocol>)connection
    ...
    NSLog(@"tpmn PBMBidRequester initWithConnection");
...
//    NSLog(@"tpmn PBMBidRequester initWithConnection %ld" , (long)[self addInteger:10 secondInt:3]);
...
- (void)requestBidsWithCompletion:(void (^)(BidResponse *, NSError *))completion {
    ...
    NSLog(@"tpmn PBMBidRequester requestBidsWithCompletion");
   if (setupError) {
    ...
    [self.connection post:requestServerURL
        ...
        NSLog(@"tpmn PBMBidRequester requestServerURL : %@",requestServerURL);
        ...
        if (serverResponse.error) {
            PBMLogInfo(@"tpmn Bid Request Error: %@", [serverResponse.error localizedDescription]);
        ...
        NSLog(@"tpmn Bid Response %@ : ", [[NSString alloc] initWithData:serverResponse.rawData encoding:NSUTF8StringEncoding]);
...
//        NSLog(@"tpmn PBMBidRequester bidResponse : %@" , [[NSString alloc] initWithData:serverResponse.rawData encoding:NSUTF8StringEncoding] );
...
- (NSString *)getRTBRequest {
    NSLog(@"tpmn PBMBidRequester getRTBRequest");
    ...
    NSLog(@"tpmn PBMBidRequester getRTBRequest openrtb %@" , params[@"openrtb"]);
 
    return params[@"openrtb"];
}

- (NSError *)findErrorInSettings {
    NSLog(@"tpmn PBMBidRequester findErrorInSettings");
