                      ...
                      configValidationBlock:(PBMAdUnitConfigValidationBlock)configValidationBlock
{
    ...
    NSLog(@"tpmn PBMAdLoadFlowController initWithBidRequesterFactory");
...
- (void)adLoader:(id<PBMAdLoaderProtocol>)adLoader loadedPrimaryAd:(id)adObject adSize:(nullable NSValue *)adSize {
    NSLog(@"tpmn PBMAdLoadFlowController loadedPrimaryAd");
 @weakify(self);
...
- (void)adLoader:(id<PBMAdLoaderProtocol>)adLoader failedWithPrimarySDKError:(nullable NSError *)error {
    NSLog(@"tpmn PBMAdLoadFlowController failedWithPrimarySDKError");
 @weakify(self);
    [self enqueueGatedBlock:^{
        @strongify(self);
        if (!self) { return; }
...
- (void)adLoaderDidWinPrebid:(id<PBMAdLoaderProtocol>)adLoader {
    NSLog(@"tpmn PBMAdLoadFlowController adLoaderDidWinPrebi1d");
  @weakify(self);
    [self enqueueGatedBlock:^{
        @strongify(self);
        if (!self) { return; }
...
- (void)adLoaderLoadedPrebidAd:(id<PBMAdLoaderProtocol>)adLoader {
    NSLog(@"tpmn PBMAdLoadFlowController adLoaderLoadedPrebidAd");
    @weakify(self);
    [self enqueueGatedBlock:^{
        @strongify(self);
        if (!self) { return; }
...
- (void)adLoader:(id<PBMAdLoaderProtocol>)adLoader failedWithPrebidError:(nullable NSError *)error {
    NSLog(@"tpmn PBMAdLoadFlowController adLoader failedWithPrebidError");
   @weakify(self);
    [self enqueueGatedBlock:^{
        @strongify(self);
        if (!self) { return; }
...
- (void)enqueueGatedBlock:(nonnull dispatch_block_t)block {
    @weakify(self);
//    printf("enqueueGatedBlock");

    dispatch_async(self.dispatchQueue, ^{
        @strongify(self);
         NSLog(@"tpmn PBMAdLoadFlowController enqueueGatedBlock");
...
- (void)refresh {
    ...
    [self enqueueGatedBlock:^{
        @strongify(self);
        if (!self) { return; }
        NSLog(@"tpmn PBMAdLoadFlowController refresh");
...
- (void)enqueueNextStepAttempt {
    NSLog(@"tpmn PBMAdLoadFlowController enqueueNextStepAttempt");
  @weakify(self);
    [self enqueueGatedBlock:^{
        @strongify(self);
        if (!self) { return; }
...
- (void)moveToNextLoadingStep {
    NSLog(@"tpmn moveToNextLoadingStep flowState : %ld  " ,self.flowState );
    
    switch (self.flowState) {
        case PBMAdLoadFlowState_Idle:
             NSLog(@"tpmn PBMAdLoadFlowState_Idle");

        case PBMAdLoadFlowState_LoadingFailed:
        {
            NSLog(@"tpmn PBMAdLoadFlowState_LoadingFailed");
        ...
        case PBMAdLoadFlowState_DemandReceived:
            NSLog(@"tpmn PBMAdLoadFlowState_DemandReceived");

            [self requestPrimaryAdServer:self.bidResponse];
            return;
            
        case PBMAdLoadFlowState_ReadyToDeploy:
            NSLog(@"tpmn PBMAdLoadFlowState_ReadyToDeploy");
...
- (void)sendBidRequest {
    ...
    NSLog(@"tpmn PBMAdLoadFlowController sendBidRequest");
...
- (void)handleBidResponse:(nullable BidResponse *)response error:(nullable NSError *)error {
    NSLog(@"tpmn PBMAdLoadFlowController handleBidResponse");
...
- (void)requestPrimaryAdServer:(nullable BidResponse *)bidResponse {
    NSLog(@"tpmn PBMAdLoadFlowController requestPrimaryAdServer");
...
- (void)loadPrebidDisplayView {
    NSLog(@"tpmn PBMAdLoadFlowController loadPrebidDisplayView");
   if (self.bidRequestError) {
        NSError * const requestError = self.bidRequestError;
        self.bidRequestError = nil;
...
- (void)markReadyToDeployAdView {
    NSLog(@"tpmn PBMAdLoadFlowController markReadyToDeployAdView");
  self.flowState = PBMAdLoadFlowState_ReadyToDeploy;
    [self enqueueNextStepAttempt];
}

- (void)deployPendingViewAndSendSuccessReport {
    NSLog(@"tpmn deployPendingViewAndSendSuccessReport\n");
