...
public class BannerView: UIView,
    ...
    @objc public init(frame: CGRect,
        ...
        Log.error("tpmn init BannerView")
        ...
        NSLog("tpmn PrebidConstants.supportedRenderingBannerAPISignals : %@ ", "")
//        NSString *strVast = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    ...
    public func bannerAdLoader(_ bannerAdLoader: PBMBannerAdLoader, loadedAdView adView: UIView, adSize: CGSize) {
        NSLog("tpmn Banner bannerAdLoader")
        deployView(adView)
    ...
    private func reportLoadingSuccess(with size: CGSize) {
        NSLog("tpmn Banner reportLoadingSuccess")
    ...
    private func reportLoadingFailed(with error: Error?) {
        NSLog("tpmn Banner reportLoadingFailed")
